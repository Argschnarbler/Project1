# Source File Name: Bacterium_05.py
# Author's Name: Jacob Meikle
# Last Modified By: Jacob Meikle
# Date Last Modified: July 29, 2013
""" 
...
class levelChanger(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.tick > self.oneSecond * 30 and self.tick < self.oneSecond * 40 :
        ...
        elif self.tick > self.oneSecond * 40 :
...
def game(level, score):
    ...
    if level == 1:
        ...
        whiteCells = [ WhiteCell() for i in range(9)]
    elif level == 3:
        #spawning enemies
    ...
    if level == 4:
        heart = Heart()
        friendSprites = pygame.sprite.OrderedUpdates(bloodStream,heart, redCells, bacteria, cursor)
    ...
    return scoreboard.score, scoreboard.lives
...
def main():
    ...
    while not donePlaying:
        ...
        if not donePlaying:
            Finalscore, Lives = game(4, Finalscore)
            if Lives > 0:
                Finalscore, Lives = game(2, Finalscore)
                if Lives > 0:
                    Finalscore, Lives = game(3, Finalscore)
                    if Lives > 0:
                        Finalscore, Lives = game(4, Finalscore)
        if Lives > 0:
            alive = True
