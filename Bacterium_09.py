...
""" 
  ...
  Version: 0.9 -  *Added a 4th level
      
  Version: 0.8 -  *Added the splash screen
...
class WhiteCell(pygame.sprite.Sprite):
    ...
    def update(self):      
        ...
        if self.level > 3:
            #randomly switch directions of white cell's up and down
            self.switchtry = random.randrange(1,51,1)
            self.switchgo = random.randrange(1,51,1)
            if self.switchtry == self.switchgo:
                self.dy *= -1
...
""" This class displays the splash screen for 3 seconds"""          
class SplashScreen(pygame.sprite.Sprite):
    ...
    def reset(self):
        self.rect.centery = screen.get_height() * 3
        self.rect.centerx = screen.get_width() * 3
...
def instructions(splash):
    ...
    if mysplash:
        splash = SplashScreen()
        splashSprite = pygame.sprite.Group(splash)
...
def main():
    ...
    while not donePlaying:
        ...
        Lives = 10
        alive = False
        #Display splash on the first run only.
            ...
            if Lives > 0 and not donePlaying:
                ...
                if Lives > 0 and not donePlaying:
                    ...
                    if Lives > 0 and not donePlaying:
                        ...
                        if Lives > 0 and  not donePlaying:
                            donePlaying, Finalscore, Lives = game(5, Finalscore, Lives)
        if not donePlaying:
            if Lives > 0:
                alive = True
